...
try:
    ...
    try:
        system(
            "python -m pip install apiclient oauth2client unidecode screeninfo turtle")  # TODO: this is likely to cause issues if python2.7 is also present. Resolve.
        system("python -m pip install --upgrade google-api-python-client")
        from apiclient import discovery
        from httplib2 import Http
        from oauth2client import client, file, tools
        from unidecode import unidecode  # to ignore accents / special characters when searching
...
# import all global variables from settings.py and functions from functions.py
import settings
import functions
import forest
...
class appMainWindow():
    def __init__(self):
        ...
        if path.isfile('./credentials/client_secret.json'):
            if path.isfile('./credentials/credentials.json'):
                try:
                    settings.drive = settings.apiConnect().connect()  # connect to the api and get the drive variable
                except:
                    pass
            else:
    ...
    def config(self):
        if path.isfile('./credentials/credentials.json'):
            ...
            if message == True:
                ...
                settings.drive = settings.apiConnect().config()
                appMainWindow()
        else:
            message = tk.messagebox.askokcancel(None,
    ...
    def uploadPopup(self, event):
        if not (
            ...
            Thread(target=functions.contextFunctions().upload, args=(self.data, target)).start()
    ...
    def starUnstar(self):
        if not (
            ...
            Thread(target=functions.contextFunctions().starUnstar, args=[self.data]).start()
        else:
            tk.messagebox.showinfo(None, "Please wait for the current operation to complete.")
    ...
    def createFolder(self, treeRef, item, newName):
        ...
        Thread(target=functions.contextFunctions().createFolder, args=(newName, parentId, treeRef)).start()
    ...
    def rename(self, treeRef, item, newName):
        self.popup.destroy()
        Thread(target=functions.contextFunctions().rename, args=(treeRef, item, newName)).start()
    ...
    def removeRestore(self, flag):
        if not (
            ...
            for i in items:
                self.data.append(settings.widget.item(i))
            Thread(target=functions.contextFunctions().removeRestore, args=(self.data, flag)).start()
        else:
            tk.messagebox.showinfo(None, "Please wait for the current operation to complete.")
    ...
    def downloadSelected(self):
        ...
        for i in items:
            self.data.append(settings.widget.item(i))
        Thread(target=functions.contextFunctions().download, args=[self.data]).start()
    ...
    def copySelected(self):
        ...
        Thread(target=functions.contextFunctions().copy, args=(self.data, destination)).start()

    def moveSelected(self):
        destination = self.selectview.item(self.selectview.selection())
        self.selectWindow.destroy()
        Thread(target=functions.contextFunctions().move, args=(self.data, destination)).start()
                        ...
                        if settings.files[i]['ownedByMe'] and not settings.files[i]['trashed']:
                            try:
                                ...
                                modified = datetime.strftime(
                                    ...
                                    '%d/%m/%y %H:%M')
                                try:
                                    attributes = functions.contextFunctions().getMimeAndSize(
                    ...
                    if settings.treeview.selection()[0] == self.trash:
                        ...
                        if settings.files[i]['trashed'] and (fileParent == None or fileParent == settings.rootID):
                            modified = datetime.strftime(
                                ...
                                '%d/%m/%y %H:%M')
                            try:
                                attributes = functions.contextFunctions().getMimeAndSize(settings.files[i]['mimeType'],
    ...
    def spawnSearchWindow(self):
        ...
        if not settings.loadingData:  # only enable the search function when the data is loaded, otherwise results may be incomplete.
            if not searchWindow:  # if search window is open already, don't reopen
                appSearchWindow(settings.rootID, 2 * w / 3, h - h / 6, x + w / 6, y + h / 12)  # jesus fucking christ.
...
class appSearchWindow():
    ...
    def starUnstar(self):
        ...
        for i in items:
            ...
            data.append(i)
        Thread(target=functions.contextFunctions().starUnstar, args=[data]).start()
        ...
        if not (
            ...
            for i in items:
                ...
                i['text'] = [j['name'] for j in settings.files + settings.folders if i['tags'][1] == j['id']][
                    0]  # search in both lists at once.
                data.append(i)
            Thread(target=functions.contextFunctions().removeRestore, args=(data, flag)).start()
        else:
            tk.messagebox.showinfo(None, "Please wait for the current operation to complete.")
            ...
            try:
                ...
                for i in items:  # item names contain their full path. Need to keep the file/folder name only. It may contain a '/' so can't just split on this.
                    ...
                    i['text'] = [j['name'] for j in settings.files + settings.folders if i['tags'][1] == j['id']][
                        0]  # search in both lists at once.
                    data.append(i)
                self.download = Thread(target=functions.contextFunctions().download, args=[data])
                self.download.start()
    ...
    def search(self,
        ...
        if not self.searchEntry.get() == "":
            ...
            #            results = []
            results = [i for i in settings.files + settings.folders if query in str(i)]
            ...
            for i in range(len(results)):
                if self.searchIsRunning:  # press on Escape switches this to False and interrupts the search results display.
                    # try:
                    try:
                        attributes = functions.contextFunctions().getMimeAndSize(results[i]['mimeType'],
                                                                                 results[i]['size'])
                    except KeyError:
                        attributes = functions.contextFunctions().getMimeAndSize(results[i]['mimeType'], None)
